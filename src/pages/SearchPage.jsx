...
export default function SearchPage() {
  ...
  const fetchResults = useCallback(async () => {
    ...
    try {
      ...
      if (subjectText.trim()) {
        url += `&subject=${encodeURIComponent(subjectText.trim())}`;
      }
      ...
      if (subjectText.trim()) {
        users = users.filter(u => {
          ...
          return subs.some(us => us.toLowerCase().includes(subjectText.trim().toLowerCase()));
        });
      }
      ...
      if (userLocation) {
        logSearch(lookingFor, userLocation.lat, userLocation.lng, subjectText, range);
      }
    } catch (err) {
  ...
  }, [lookingFor, userLocation, range, maxPrice, subjectText, locationStep]);
            ...
            <div className="max-w-5xl mx-auto p-4 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-2">
                    <button onClick={() => setRange(Math.max(1, range - 10))} className="w-10 h-10 bg-[#121212] border border-gray-700 rounded-xl flex items-center justify-center hover:border-[#FF6B2B] transition cursor-pointer"><Minus size={16} /></button>
                    ...
                    <button onClick={() => setRange(Math.min(50, range + 10))} className="w-10 h-10 bg-[#121212] border border-gray-700 rounded-xl flex items-center justify-center hover:border-[#FF6B2B] transition cursor-pointer"><Plus size={16} /></button>
                  </div>
                </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {['Mathematics', 'Physics', 'Chemistry', 'English', 'Biology', 'Computer Science', 'Hindi', 'Economics', 'Accounts'].map(sub => (
                  <button key={sub} onClick={() => setSubjectText(sub)}
                    className={`px-2.5 py-1 rounded-full text-[11px] font-medium transition cursor-pointer border ${subjectText === sub ? 'bg-[#FF6B2B] text-white border-[#FF6B2B]' : 'bg-[#121212] text-gray-500 border-gray-800 hover:border-gray-600 hover:text-gray-300'}`}>
                    {sub}
                  </button>
                ))}
              </div>
